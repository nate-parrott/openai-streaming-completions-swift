...
public struct OpenAIAPI {
    ...
    public func complete(_ completionRequest: CompletionRequest) async throws -> String {
        let request = try createRequest(completionRequest: completionRequest)
    ...
    public func completeStreaming(_ completionRequest: CompletionRequest) throws -> StreamingCompletion {
        // TODO: Use server-sent events (https://nickarner.com/notes/working-with-server-sent-events-in-swift-november-16-2021/)
        var cr = completionRequest
        cr.stream = true

        let request = try createRequest(completionRequest: cr)
...
private extension OpenAIAPI {
    ...
    func createRequest(completionRequest: CompletionRequest) throws -> URLRequest {
